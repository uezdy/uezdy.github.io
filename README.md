...
| Путь | Описание |
| ---- | -------- |
| `data/groups.json` | Список групп для экспорта (`slug` + `chat` или `id`) |
...
```json
{
  "groups": [
    ...
    { "slug": "private_group", "id": -1001234567890 }
  ]
}
```

- **slug** — короткое имя для URL сайта (`/lepelskiy/`, `/lepelskiy/topic/107/messages/1/`)
- **chat** — @username, ссылка `t.me/...` или числовой id в виде строки
- **id** — числовой id группы в Telegram (альтернатива `chat`, удобно для приватных групп)

**Как указать группу:**

| Поле | Вариант          | Пример                  |
| ---- | ---------------- | ----------------------- |
| `chat` | Публичная группа | `@my_group`             |
| `chat` | Числовой id      | `-1001234567890`        |
| `chat` | Ссылка           | `https://t.me/my_group` |
| `id`   | Числовой id      | `-1001234567890`        |

Для приватной группы без `@username` удобнее поле `id`. Его можно узнать через бота вроде `@userinfobot` или из ссылки вида `t.me/c/1234567890/1` → id будет `-1001234567890`.
